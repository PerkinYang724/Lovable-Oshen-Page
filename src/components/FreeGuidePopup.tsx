...
const FreeGuidePopup = () => {
  ...
  useEffect(() => {
    console.log('FreeGuidePopup: Component mounted');
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    ...
    console.log('FreeGuidePopup: popupShown in sessionStorage:', popupShown);

    if (!popupShown && !hasShown) {
      console.log('FreeGuidePopup: Setting timer to show popup in 800ms');
      // Delay to ensure page is fully rendered and React has hydrated
      const timer = setTimeout(() => {
        console.log('FreeGuidePopup: Timer fired, showing popup');
        setIsOpen(true);
        setHasShown(true);
      ...
      console.log('FreeGuidePopup: Popup already shown or hasShown is true, skipping');
    }
  }, [hasShown, mounted]);
